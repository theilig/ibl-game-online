...
trait ParkEffectTable {
   ...
   *  @param effect Database column effect SqlType(ENUM), Length(9,false) */
  case class ParkEffectRow(parkEffectId: Int, parkId: Int, effect: String)
  /** GetResult implicit for fetching ParkEffectRow objects using plain SQL queries */
  implicit def GetResultParkEffectRow(implicit e0: GR[Int], e1: GR[String]): GR[ParkEffectRow] = GR{
    prs => import prs._
  ...
  class ParkEffect(_tableTag: Tag) extends profile.api.Table[ParkEffectRow](_tableTag, Some("IblGame"), "ParkEffect") {
    ...
    def ? = ((Rep.Some(parkEffectId), Rep.Some(parkId), Rep.Some(effect))).shaped.<>({r=>import r._; _1.map(_=> ParkEffectRow.tupled((_1.get, _2.get, _3.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    /** Database column effect SqlType(ENUM), Length(9,false) */
    val effect: Rep[String] = column[String]("effect", O.Length(9,varying=false))

...
trait CloudCoverTable {
   ...
   *  @param cover Database column cover SqlType(ENUM), Length(13,false)
   *  @param rank Database column rank SqlType(INT)
   *  @param frequency Database column frequency SqlType(INT) */
  case class CloudCoverRow(cloudCoverId: Int, geographicLocationId: Int, gameMonth: String, cover: String, rank: Int, frequency: Int)
  /** GetResult implicit for fetching CloudCoverRow objects using plain SQL queries */
  implicit def GetResultCloudCoverRow(implicit e0: GR[Int], e1: GR[String]): GR[CloudCoverRow] = GR{
    prs => import prs._
  ...
  class CloudCover(_tableTag: Tag) extends profile.api.Table[CloudCoverRow](_tableTag, Some("IblGame"), "CloudCover") {
    ...
    def ? = ((Rep.Some(cloudCoverId), Rep.Some(geographicLocationId), Rep.Some(gameMonth), Rep.Some(cover), Rep.Some(rank), Rep.Some(frequency))).shaped.<>({r=>import r._; _1.map(_=> CloudCoverRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    /** Database column cover SqlType(ENUM), Length(13,false) */
    val cover: Rep[String] = column[String]("cover", O.Length(13,varying=false))

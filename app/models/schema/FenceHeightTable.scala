...
trait FenceHeightTable {
  ...
  case class FenceHeightRow(fenceHeightId: Int, parkId: Int, location: String, fenceHeight: Int)
  /** GetResult implicit for fetching FenceHeightRow objects using plain SQL queries */
  implicit def GetResultFenceHeightRow(implicit e0: GR[Int], e1: GR[String]): GR[FenceHeightRow] = GR{
    prs => import prs._
  ...
  class FenceHeight(_tableTag: Tag) extends profile.api.Table[FenceHeightRow](_tableTag, Some("IblGame"), "FenceHeight") {
    ...
    def ? = ((Rep.Some(fenceHeightId), Rep.Some(parkId), Rep.Some(location), Rep.Some(fenceHeight))).shaped.<>({r=>import r._; _1.map(_=> FenceHeightRow.tupled((_1.get, _2.get, _3.get, _4.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))

...
trait TemperatureTable {
   ...
   *  @param temp Database column temp SqlType(ENUM), Length(4,false)
   *  @param rank Database column rank SqlType(INT)
   *  @param frequency Database column frequency SqlType(INT) */
  case class TemperatureRow(temperatureId: Int, geographicLocationId: Int, timeOfDay: String, gameMonth: String, temp: String, rank: Int, frequency: Int)
  /** GetResult implicit for fetching TemperatureRow objects using plain SQL queries */
  implicit def GetResultTemperatureRow(implicit e0: GR[Int], e1: GR[String]): GR[TemperatureRow] = GR{
    prs => import prs._
  ...
  class Temperature(_tableTag: Tag) extends profile.api.Table[TemperatureRow](_tableTag, Some("IblGame"), "Temperature") {
    ...
    def ? = ((Rep.Some(temperatureId), Rep.Some(geographicLocationId), Rep.Some(timeOfDay), Rep.Some(gameMonth), Rep.Some(temp), Rep.Some(rank), Rep.Some(frequency))).shaped.<>({r=>import r._; _1.map(_=> TemperatureRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get, _7.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    /** Database column temp SqlType(ENUM), Length(4,false) */
    val temp: Rep[String] = column[String]("temp", O.Length(4,varying=false))

...
trait DistanceTable {
   ...
   *  @param location Database column location SqlType(ENUM), Length(4,false)
  ...
  case class DistanceRow(distanceId: Int, parkId: Int, location: String, windType: String, fenceDistance: Int)
  /** GetResult implicit for fetching DistanceRow objects using plain SQL queries */
  implicit def GetResultDistanceRow(implicit e0: GR[Int], e1: GR[String]): GR[DistanceRow] = GR{
    prs => import prs._
  ...
  class Distance(_tableTag: Tag) extends profile.api.Table[DistanceRow](_tableTag, Some("IblGame"), "Distance") {
    ...
    def ? = ((Rep.Some(distanceId), Rep.Some(parkId), Rep.Some(location), Rep.Some(windType), Rep.Some(fenceDistance))).shaped.<>({r=>import r._; _1.map(_=> DistanceRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    /** Database column location SqlType(ENUM), Length(4,false) */
    val location: Rep[String] = column[String]("location", O.Length(4,varying=false))

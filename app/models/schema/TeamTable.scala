...
trait TeamTable {
   ...
   *  @param abbrev Database column abbrev SqlType(VARCHAR), Length(3,true) */
  case class TeamRow(teamId: Int, city: String, nickname: String, geographicLocationId: Int, homeParkId: Int, abbrev: String)
  ...
  implicit def GetResultTeamRow(implicit e0: GR[Int], e1: GR[String]): GR[TeamRow] = GR{
    prs => import prs._
    TeamRow.tupled((<<[Int], <<[String], <<[String], <<[Int], <<[Int], <<[String]))
  }
  ...
  class Team(_tableTag: Tag) extends profile.api.Table[TeamRow](_tableTag, Some("IblGame"), "Team") {
    def * = (teamId, city, nickname, geographicLocationId, homeParkId, abbrev) <> (TeamRow.tupled, TeamRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(teamId), Rep.Some(city), Rep.Some(nickname), Rep.Some(geographicLocationId), Rep.Some(homeParkId), Rep.Some(abbrev))).shaped.<>({r=>import r._; _1.map(_=> TeamRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    /** Database column abbrev SqlType(VARCHAR), Length(3,true) */
    val abbrev: Rep[String] = column[String]("abbrev", O.Length(3,varying=true))
  }
